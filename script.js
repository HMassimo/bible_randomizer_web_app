// A smaller subset of Bible data to keep the file size reasonable for a web app.
// For a full version, this would be a much larger file.
// The complete verse count data for every chapter in the Catholic Bible canon.
const BIBLE_DATA = {
  Genesis: [
  ...
  Tobit: [22, 14, 17, 25, 22, 17, 16, 21, 6, 13, 18, 22, 18, 15],
  Judith: [28, 28, 10, 15, 24, 21, 32, 36, 18, 23, 23, 20, 31, 19, 13, 25],
  Esther: [22, 23, 15, 17, 14, 14, 10, 21, 32, 3, 12, 6, 18, 19, 16, 24],
  "1 Maccabees": [
    64, 70, 60, 61, 68, 63, 50, 32, 73, 89, 70, 53, 53, 49, 41, 24,
  ],
  "2 Maccabees": [36, 32, 40, 50, 27, 31, 42, 37, 29, 38, 38, 29, 24, 32, 39],
  Job: [
  ...
  Psalms: [
    ...
    8, 12, 11, 10, 13, 20, 7, 35, 36, 5, 8, 12, 18, 5, 6, 5, 20, 72, 13, 19, 16,
    8, 18, 12, 13, 17, 7, 18, 52, 7, 8, 9, 8, 11, 22, 19, 14, 12, 15, 23, 10,
    20, 13, 9, 9, 12, 20, 28, 22, 35, 13, 10, 12, 4, 7, 48, 12, 10, 8, 12, 8,
    20, 23, 176, 8, 9, 5, 8, 7, 8, 6, 7, 7, 6, 6, 5, 6, 6,
  ],
  Proverbs: [
  ...
  Wisdom: [
    16, 24, 19, 20, 22, 22, 29, 21, 18, 21, 26, 27, 19, 31, 19, 29, 20, 25, 22,
  ],
  Sirach: [
    30, 18, 31, 31, 15, 37, 36, 19, 18, 31, 34, 17, 26, 26, 20, 22, 32, 33, 30,
    29, 35, 29, 37, 27, 33, 31, 30, 26, 28, 25, 24, 24, 22, 31, 26, 31, 31, 32,
    19, 32, 24, 22, 28, 30, 26, 28, 27, 28, 25, 31, 27,
  ],
  Isaiah: [
    31, 22, 26, 6, 30, 13, 25, 22, 21, 34, 16, 6, 22, 32, 9, 14, 14, 7, 25, 6,
    17, 25, 18, 23, 12, 21, 13, 29, 24, 33, 9, 20, 24, 17, 10, 22, 38, 22, 8,
    31, 29, 25, 28, 28, 25, 13, 15, 22, 26, 11, 23, 15, 12, 17, 13, 21, 22, 12,
    12, 17, 25, 12, 24,
  ],
  Jeremiah: [
  ...
  Ezekiel: [
    28, 10, 27, 17, 17, 14, 27, 18, 11, 22, 25, 28, 23, 23, 8, 63, 17, 24, 32,
    14, 49, 32, 31, 49, 27, 17, 21, 36, 26, 21, 26, 18, 32, 33, 31, 15, 38, 28,
    23, 29, 49, 26, 20, 27, 31, 25, 24, 35, 23,
  ],
  Daniel: [21, 49, 30, 37, 31, 28, 28, 27, 27, 21, 45, 12, 64, 42],
...
chapterBtn.addEventListener("click", () => {
  const bookNames = Object.keys(BIBLE_DATA);
  const randomBookName =
    bookNames[Math.floor(Math.random() * bookNames.length)];
  const numChapters = BIBLE_DATA[randomBookName].length;
  const randomChapter = Math.floor(Math.random() * numChapters) + 1;
  resultText.textContent = `${randomBookName} ${randomChapter}`;
});
...
verseBtn.addEventListener("click", () => {
  ...
  const randomBookName =
    bookNames[Math.floor(Math.random() * bookNames.length)];
  const chapterLengths = BIBLE_DATA[randomBookName];
  const randomChapterIndex = Math.floor(Math.random() * chapterLengths.length);
  const numVerses = chapterLengths[randomChapterIndex];
  const randomChapter = randomChapterIndex + 1;
  const randomVerse = Math.floor(Math.random() * numVerses) + 1;
  resultText.textContent = `${randomBookName} ${randomChapter}:${randomVerse}`;
});
